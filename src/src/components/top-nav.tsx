import { HomeIcon, PlusIcon, InformationCircleIcon, CalculatorIcon, ViewfinderCircleIcon, SunIcon } from '@heroicons/react/20/solid'
...
import FireDangerReader from './fireDangerReader';
...
const TopNav = () => {
    ...
    const showItasc = useNoteStore((state: INoteStore) => state.showItaskForm);
    const updateAddress = useLocationStore((state: ILocationStore) => state.setAddress);
    const showFireInfo = useNoteStore((state: INoteStore) => state.showFireReading);
    const showFireDrawer = useNoteStore((state: INoteStore) => state.ShowFireInfo);
    ...
    return (
        <>
            <Drawer title="Fire Reading" isOpen={showFireDrawer} onClose={showFireInfo} key="fireInfo" subHeading="Critical information about your current fire behviour based on the inputed weather params">
                <>
                <FireDangerReader/>
                </>
            </Drawer>
            ...
            {currentNote.Id > 0 ? (<>
                <span className="isolate flex mb-5 p-1">
                    ...
                    <button type="button" onClick={showItasc}
                        className="relative inline-flex items-center rounded-l-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 ring-1 ring-inset ring-gray-300 hover:bg-gray-50 focus:z-10"                    >
                        <SunIcon aria-hidden="true" className="-ml-1 mr-0.5 h-5 w-5 text-gray-400" />WEATHER
                    </button>
                </span>
                <span className="isolate flex mb-5 p-1">
                    <button type="button" onClick={showFireInfo}
                        className="relative inline-flex items-center rounded-l-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 ring-1 ring-inset ring-gray-300 hover:bg-gray-50 focus:z-10"                    >
                        <PlusIcon aria-hidden="true" className="-ml-1 mr-0.5 h-5 w-5 text-gray-400" />FIRE
                    </button>
